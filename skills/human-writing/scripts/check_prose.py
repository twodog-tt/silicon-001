...
def analyze(text: str) -> dict:
    """结构化检查结果（Qingting 补丁：供 human_writing_gate 调用）。"""
...
def main() -> int:
    ...
    result = analyze(text)
    if result["total_han"] == 0:
        print("没有检测到汉字。", file=sys.stderr)
        return 2

    stats = result["stats"]
    print(f"汉字数 {result['total_han']}")
    print(
        f"翻案句 {stats.get('pivots', 0)}，翻案腔变形 {stats.get('semantic_pivots', 0)}，"
        f"同构排比 {stats.get('anaphoras', 0)}，名词化 {stats.get('nominalizations', 0)}，"
        f"黑话 {stats.get('jargon', 0)}，硬停词 {stats.get('hard_stops', 0)}，"
        f"模型路标 {stats.get('road_signs', 0)}，需辨语境词 {stats.get('context_jargon', 0)}，"
        f"抒情词 {stats.get('lyric', 0)}，洞察路标 {stats.get('markers', 0)}，"
        f"长前置成分 {stats.get('left_branches', 0)}，重定语句 {stats.get('dense_de', 0)}"
    )
